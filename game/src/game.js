...
const PLANETS=[
 ...
 {id:'magmar', name:'MAGMAR',  unlock:600,  rings:30, hardMul:1.5, heatMul:1.9, valueMul:1.7, gasChance:0.06,
...
function loadMeta(){try{const j=JSON.parse(localStorage.getItem(SAVE_KEY));if(j&&j.v===2){
  ...
  if(!j.daily)j.daily=null;return j;}}catch(e){}
...
function stats(){
  let power=30,speed=230,energyMax=130,energyRegen=0,coolRate=0,heatGen=7,magnet=68,valueMul=1,crit=0,drones=0;
...
function genTile(depthM,ring,sec){let rock='dirt';
...
function applyLang(){
  ...
  set('btnRetry','retry');set('btnGoMenu','menu');
