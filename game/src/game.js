...
const PLANETS=[
 ...
 {id:'abyss',  name:'ABYSS',   unlock:9000, rings:40, hardMul:2.6, heatMul:1.3, valueMul:3.6, gasChance:0.05, caveChance:0.05,
...
function loadMeta(){try{const j=JSON.parse(localStorage.getItem(SAVE_KEY));if(j&&j.v===2){
  ...
  if(!j.daily)j.daily=null;if(!j.cosmetics)j.cosmetics={owned:['default'],equipped:'default'};return j;}}catch(e){}
...
const ACH=[
 ...
 {id:'starmap',ic:'🌌', de:['Sternenkarte','Schalte alle 5 Planeten frei'],  en:['Star Map','Unlock all 5 planets'],          cond:()=>meta.unlockedPlanets.length>=5},
...
function genTile(depthM,ring,sec){
  // core guardian: the 2 innermost rings are a super-hard boss layer with guaranteed loot
  if(ring<2){const bhp=HARD.boss*P.hardMul;return{rock:'boss',res:ring===0?'artifact':'core',boss:true,hp:bhp,maxhp:bhp};}
  let rock='dirt';
...
function ensureDaily(){const ds=new Date().toDateString();if(meta.daily&&meta.daily.date===ds)return;
  const r=dayRng(),kinds=[{type:'depth',target:30+Math.floor(r()*45),reward:200},
    {type:'loot',target:600+Math.floor(r()*2000),reward:300},{type:'runs',target:3+Math.floor(r()*5),reward:250}];
...
function buildPlanets(){const g=document.getElementById('planetCards');g.innerHTML='';
  PLANETS.forEach(p=>{const own=meta.unlockedPlanets.includes(p.id),sel=meta.planet===p.id;
    ...
    card.innerHTML='<span class="globe" style="'+gstyle+'"></span>'+
      '<span class="pt"><b>'+p.name+'</b><span>Härte ×'+p.hardMul.toFixed(1)+' · Loot ×'+p.valueMul.toFixed(1)+' · Tiefe '+p.rings+'</span></span>'+
      '<span class="ps" style="color:'+(sel?'#2de2e6':own?'#12d9b0':'#ffd23f')+'">'+(sel?'★ AKTIV':own?'WÄHLEN':p.unlock+'$ 🔒')+'</span>';
...
function scene(){
  ...
  for(let ring=ringLo;ring<=ringHi;ring++){const rin=R_CORE+ring*TILE,rout=rin+TILE;
    ...
    for(let off=-range;off<=range;off++){const sec=((curSec+off)%n+n)%n;
      ...
      o.strokeStyle='rgba(0,0,0,0.30)';o.lineWidth=1;o.stroke();
