...
const L={de:{
  ...
  goSub:'Kein Extract = kein Cash. Sprit &amp; Hitze regenerieren nicht — geh rechtzeitig HOME.',
  ...
  reasonFuel:'SPRIT LEER', reasonHeat:'ÜBERHITZT', lootLost:'Loot verloren',
  ...
  goSub:'No extract = no cash. Fuel &amp; heat do not regen — head HOME in time.',
  ...
  reasonFuel:'OUT OF FUEL', reasonHeat:'OVERHEATED', lootLost:'Loot lost',
...
function applyLang(){
  ...
  set('titleSub','titleSub');set('hint','hint',true);set('goSub','goSub',true);set('goLostLbl','lootLost');
...
function scene(){
  ...
  const neb=o.createRadialGradient(LW*0.5,LH*0.32,0,LW*0.5,LH*0.32,LH*0.7);
  ...
  for(let i=0;i<80;i++){const px=(i*137.5)%LW|0,py=(i*89.3)%LH|0;
  ...
  for(let ring=ringLo;ring<=ringHi;ring++){const rin=R_CORE+ring*TILE,rout=rin+TILE;
    ...
    for(let off=-range;off<=range;off++){const sec=((curSec+off)%n+n)%n;
      ...
      o.strokeStyle='rgba(255,255,255,0.12)';o.beginPath();o.moveTo(p1[0]*S1,p1[1]*S1);o.lineTo(p2[0]*S1,p2[1]*S1);o.stroke();
...
function draw(){scene();
  ...
  {const vg=ctx.createRadialGradient(W/2,H*0.46,H*0.28,W/2,H*0.5,H*0.75);
