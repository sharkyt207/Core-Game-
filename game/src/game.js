...
function buildPlanets(){const g=document.getElementById('planetCards');g.innerHTML='';
  PLANETS.forEach(p=>{const own=meta.unlockedPlanets.includes(p.id),sel=meta.planet===p.id;
    ...
    const globe='radial-gradient(circle at 34% 30%, '+p.ground[1]+', '+p.ground[0]+' 55%, '+p.ground[2]+' 100%)';
    card.innerHTML='<span class="globe" style="background:'+globe+';border:2px solid '+p.core+';"></span>'+
...
function drawHUD(){const c=ctx,M=12,top=Math.max(14,H*0.05);c.imageSmoothingEnabled=true;
  ...
  c.fillStyle=(run.energy/S.energyMax<0.2)?'#ff4d4d':T.fuel;c.fillRect(bx+2,by+2,(bw-4)*Math.max(0,run.energy/S.energyMax),10);
