...
function stats(){
  let power=30,speed=230,energyMax=100,energyRegen=9,coolRate=16,heatGen=11,magnet=68,valueMul=1,crit=0,drones=0;
  const abilities={};
  for(const id of meta.skills){const e=SKILLMAP[id]&&SKILLMAP[id].eff;if(!e)continue;
    ...
    heatGen-=e.dh||0;coolRate+=e.dc||0;magnet+=e.dm||0;valueMul+=e.dv||0;crit+=e.crit||0;
    if(e.auto)magnet+=120;if(e.drone)drones++;if(e.ability)abilities[e.ability]=1;}
  return{power,speed,energyMax,energyRegen,coolRate,heatGen:Math.max(3,heatGen),magnet,valueMul,crit,drones,abilities,
